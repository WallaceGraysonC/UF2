...
enum NetworkMessage: Codable {
    ...
    enum CodingKeys: String, CodingKey { case type, state, action, playerID }

    private enum Kind: String, Codable { case stateSync, action, requestNextRound }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .requestNextRound:
            try container.encode(Kind.requestNextRound, forKey: .type)
        }
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        case .requestNextRound:
            self = .requestNextRound
        }
    }
}

/// Coordinates a live GKMatch with the local BlackjackEngine. The player who
...
final class MultiplayerMatch: ObservableObject {
    ...
    init(isHost: Bool, engine: BlackjackEngine?, localPlayerID: String) {
        self.isHost = isHost
        self.engine = engine
    ...
    func sendAction(_ action: PlayerAction) {
        if isHost {
            applyToEngine(action, playerID: localPlayerID)
            broadcastState()
        } else {
    ...
    func requestNextRound() {
        if isHost {
            ...
            if let data = try? JSONEncoder().encode(NetworkMessage.requestNextRound) {
                gameCenter.send(data)
            }
        }
    }

    /// Every player's hand is public in blackjack, so -- unlike Hold'em hole
    /// cards -- one identical broadcast is safe for every peer; only the
    /// dealer's hole card needs masking, which `snapshot(for:)` already does.
    ...
    private func applyToEngine(_ action: PlayerAction, playerID: String) {
        switch action {
        case .placeBet(let amount):
            engine?.placeBet(amount, for: playerID)
        case .insurance(let accept):
            engine?.decideInsurance(accept, for: playerID)
    ...
    private func handle(data: Data, from senderID: String) {
        ...
        case .action(let action, let playerID):
            ...
            applyToEngine(action, playerID: playerID)
            broadcastState()
        case .requestNextRound:
            guard isHost else { return }
            engine?.startNextRound()
